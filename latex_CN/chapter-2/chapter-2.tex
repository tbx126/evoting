...
\begin{enumerate}
    ...
    \item \textbf{ZKP3（计票解密正确性）}：证明对每个候选人 $j$，公开结果点 $R_j = n_j \cdot G$ 等于在私钥 $sk$ 下对聚合密文解密得到的点，且 $sk$ 与公钥 $PK = sk \cdot G$ 一致。整数票数通过离散对数恢复在链下计算并单独提交；该电路证明的是曲线点表示的正确性，而非直接证明整数值。该电路必须独立存在，因为其输入依赖投票结束后的聚合密文状态。
\end{enumerate}
