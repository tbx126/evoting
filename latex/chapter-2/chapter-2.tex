...
\begin{itemize}
    ...
    w = (&\mathit{candidateId},\; \mathit{salt},\; r_0, \dots, r_{N-1},\\
         &\mathit{c1X}_0,\; \mathit{c1Y}_0,\; \mathit{c2X}_0,\; \mathit{c2Y}_0,\; \dots,\; \mathit{c1X}_{N-1},\; \mathit{c1Y}_{N-1},\; \mathit{c2X}_{N-1},\; \mathit{c2Y}_{N-1})
    \end{split}
    \end{gather*}
    ...
    \begin{equation*}
        ...
        \end{split}
    \end{equation*}
    where $R^{X}_j$, $R^{Y}_j$ are the coordinates of the result point $\mathbf{results}[j] \cdot G$ for candidate $j$, and $\mathit{totalVotes}$ is the expected sum of all candidate totals.
\end{itemize}
