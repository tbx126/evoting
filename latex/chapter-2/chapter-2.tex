...
\begin{itemize}
    ...
    \begin{equation*}
        ...
        \end{split}
    \end{equation*}
    where $R^{X}_j$, $R^{Y}_j$ are the coordinates of the result point $\mathbf{results}[j] \cdot G$ for candidate $j$, and $\mathit{totalVotes}$ is the expected sum of all candidate totals (corresponding to $V$ in Chapter 1).
\end{itemize}
...
In this design, a commitment acts as a binding receipt for hidden ballot data. The voter computes a hash from candidate ID and salt, then publishes only the digest. Later edits to either secret value would change that digest, so the original claim cannot be silently rewritten.
