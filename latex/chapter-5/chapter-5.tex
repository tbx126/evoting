...
\midrule
Hardhat contract/integration tests & 32/32 & about 15 s & \texttt{npm.cmd test} \\
...
Groth16 proof generation is the most computationally intensive client-side operation. It involves multi-scalar multiplication over the proving key, which scales with the number of circuit constraints. For the current configuration ($N = 2$), the project test suite reports proof generation times of approximately 3--8 seconds per vote proof and 4--10 seconds per tally proof when executed in Node.js (snarkjs, single-threaded). Browser-based execution using snarkjs WASM is expected to exhibit similar or moderately longer latencies depending on device hardware.
...
\begin{itemize}
    ...
    \item \textbf{Candidate cardinality mismatch}: circuits are compiled for two candidates while contract permits dynamic candidate count; deploying with more than two candidates will cause proof verification failures.
