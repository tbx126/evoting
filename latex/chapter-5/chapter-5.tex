...
\begin{itemize}
    ...
    \item \textbf{Discrete-log recovery} during tally decryption uses a baby-step giant-step algorithm bounded by the maximum expected vote count per candidate. For tallies up to approximately $2^{20}$ ($\sim$1M votes), this requires $O(\sqrt{V_{max}})$ precomputed table entries, which is practical. Beyond this range, the brute-force search becomes a bottleneck.
