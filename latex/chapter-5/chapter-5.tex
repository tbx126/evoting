...
For a voter casting a ballot, the browser must load approximately 10.06~MiB of proof artifacts (WASM + zkey). This is a one-time download per session and can be cached. The administrator performing a tally loads approximately 9.26~MiB. These sizes are consistent with typical Groth16 deployments at this constraint scale and are within practical browser download budgets for modern connections.
...
\midrule
\texttt{Voting} & 10{,}168 bytes (9.9 KB) & Main election logic with integrated voter registration; largest contract \\
...
The total deployed bytecode across all three contracts is 14{,}516 bytes. The Voting contract is the largest at about 41\% of the EIP-170 limit, leaving substantial headroom for additional features if needed. The Groth16 verifier contracts remain compact because snarkjs generates optimized inline pairing-check code.

\subsection{Scalability Discussion}

The system's scalability characteristics vary by component. On-chain gas per vote (\texttt{castVote}) is essentially constant regardless of how many votes have already been cast, because each vote is independently verified and stored. Let $B_{dlog}$ denote the discrete-log search bound used by the tally client. In the current implementation, $B_{dlog}=V+100$ where $V$ is total votes cast. Several operations scale with election size:
...
The audit design uses commitment leaves directly and derives internal nodes with Keccak256. Deterministic event ordering ensures that independent auditors can reconstruct the same root given the same event set.
...
\begin{itemize}
    \item \textbf{[P1] Result binding gap}: contract verifies tally proof but does not strictly bind the submitted integer result vector to all public signals used by the circuit. This can allow inconsistencies if external checks are not enforced.
