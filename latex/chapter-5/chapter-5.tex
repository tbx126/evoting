...
\midrule
Hardhat contract/integration tests & 32/32 & about 36 s & \texttt{npm test} \\
Python cryptography unit tests & 43/43 & 21.31 s & \texttt{python -m pytest tests/ -q} \\ 
\bottomrule
\end{tabular}
\end{table}

Both suites completed with zero failures, for a combined 75 passing tests. This does not prove security in adversarial deployment, but it gives strong evidence that the implementation claims in Chapters 3--4 are executable and internally consistent on a fresh local run.
...
To contextualize these costs: at a gas price of 30~gwei and an ETH price of \$2{,}500, one \texttt{castVote} transaction costs approximately 0.0096--0.0102~ETH (\$24--\$26), and one \texttt{updateTallyResults} costs approximately 0.0093--0.0099~ETH (\$23--\$25). The Ethereum block gas limit (30M gas) can accommodate roughly 88--95 vote transactions per block if they were the only transactions, although in practice, block space is shared. (Gas price and ETH price figures are illustrative values used at the time of evaluation; actual costs vary with network conditions.)
...
\begin{itemize}
    ...
    \item \textbf{Discrete-log recovery} during tally decryption uses runtime baby-step giant-step with complexity $O(\sqrt{B_{dlog}})$. No persistent global precomputed table is required; instead, a temporary baby-step map of size $O(\sqrt{B_{dlog}})$ is built in memory during tally. Since per-candidate tallies are bounded above by $V$, the current bound $B_{dlog}=V+100$ is sufficient for correctness but may become a client-side bottleneck for very large elections.
    ...
    \item \textbf{Circuit constraint scaling}: increasing the number of candidates from $N = 2$ to $N = 10$ would increase vote proof constraints approximately 4.8$\times$ (from $\sim$15k to $\sim$73k), proportionally increasing proof generation time and proving key size.
\end{itemize}
...
DLog recovery (off-chain) & $O(\sqrt{B_{dlog}})$ & Current admin client uses $B_{dlog}=V+100$ \\
