...
\midrule
Hardhat contract/integration tests & 33/33 & about 6 s & \texttt{npm test} \\
...
Artifact & Size & Role \\
\midrule
\texttt{vote\_proof.wasm} & 2.94 MB & Witness generator for vote proof \\
\texttt{vote\_proof\_final.zkey} & 7.11 MB & Proving key for vote proof \\
\texttt{tally\_proof.wasm} & 143.0 KB & Witness generator for tally proof \\
\texttt{tally\_proof\_final.zkey} & 9.12 MB & Proving key for tally proof \\
\texttt{vote\_proof.r1cs} & 2.79 MB & Constraint system (build-time only) \\
\texttt{tally\_proof.r1cs} & 3.24 MB & Constraint system (build-time only) \\
\bottomrule
\end{tabular}
\end{table}

For a voter casting a ballot, the browser must load approximately 10.0~MB of proof artifacts (WASM + zkey). This is a one-time download per session and can be cached. The administrator performing a tally loads approximately 9.3~MB. These sizes are consistent with typical Groth16 deployments at this constraint scale and are within practical browser download budgets for modern connections.
...
Table \ref{tab:gas-costs} reports the estimated gas consumption for each major on-chain operation, broken down by component. These estimates are derived from EVM opcode pricing (EIP-1108 precompile costs \cite{eip1108}, EIP-2929 cold/warm storage access costs \cite{eip2929}) applied to the contract logic in \texttt{Voting.sol}.
...
\midrule
\texttt{castVote} (voter) & 315{,}000--340{,}000 & Groth16 verification ($\sim$181k) + about 6 cold SSTOREs ($\sim$120k) + one warm state update + calldata \\
\texttt{updateTallyResults} (admin) & 310{,}000--330{,}000 & Groth16 verification ($\sim$181k) + result loop SSTORE + event \\
\texttt{addCandidates} & $\sim$80{,}000 & Two candidate SSTOREs + candidateCount SSTORE + two events \\
...
To contextualize these costs: at a gas price of 30~gwei and an ETH price of \$2{,}500, one \texttt{castVote} transaction costs approximately 0.00945--0.0102~ETH (\$24--\$26), and one \texttt{updateTallyResults} costs approximately 0.0093--0.0099~ETH (\$23--\$25). The Ethereum block gas limit (30M gas) can accommodate roughly 88--95 vote transactions per block if they were the only transactions, although in practice, block space is shared. (Gas price and ETH price figures are illustrative values used at the time of evaluation; actual costs vary with network conditions.)

\subsection{Proof Generation Latency}

Groth16 proof generation is the most computationally intensive client-side operation. It involves multi-scalar multiplication over the proving key, which scales with the number of circuit constraints. For the current configuration ($N = 2$), the project test suite reports proof generation times of approximately 3--8 seconds per vote proof and 4--10 seconds per tally proof when executed in Node.js (snarkjs, single-threaded) on a mid-range laptop (Intel Core i7, 16 GB RAM). Browser-based execution using snarkjs WASM is expected to exhibit similar or moderately longer latencies depending on device hardware.
...
\midrule
\texttt{Voting} & 9{,}481 bytes (9.3 KB) & Main election logic with integrated voter registration; largest contract \\
...
The total deployed bytecode across all three contracts is 13{,}829 bytes. The Voting contract is the largest at 39\% of the EIP-170 limit, leaving substantial headroom for additional features if needed. Merging voter registration into the Voting contract increased its size by 983 bytes while eliminating two separate deployments. The Groth16 verifier contracts are compact because snarkjs generates optimized inline pairing-check code.
...
Code review identified five gaps that affect full assurance. Gaps are classified by impact: \textbf{P1} (high impact, fix before any public deployment) and \textbf{P2} (medium impact, fix before production use):
\begin{itemize}
    ...
    \item \textbf{[P1] Audit root cross-check gap}: voter-side inclusion verification should compare bundle root with on-chain stored root to prevent acceptance of a self-consistent but unauthorized bundle.
    \item \textbf{[P2] Candidate cardinality mismatch}: circuits are compiled for two candidates while contract permits dynamic candidate count; deploying with more than two candidates will cause proof verification failures.
    \item \textbf{[P2] Merkle root mutability}: a direct root-update function exists without strict lifecycle gating; this increases governance attack surface.
\end{itemize}
