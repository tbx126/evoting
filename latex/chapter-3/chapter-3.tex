...
\begin{equation}
S = \{Created, Active, Ended, Tallied\}
\end{equation}

Allowed transitions are:
\begin{equation}
Created \rightarrow Active \rightarrow Ended \rightarrow Tallied
\end{equation}
...
\begin{equation}
C1_i = r_i G, \quad C2_i = m_i G + r_i PK
\end{equation}
where $r_i$ is random nonce, $G$ is generator, and $PK$ is administrator public key.
...
\begin{equation}
H_{ct} = Poseidon(c1X_0,c1Y_0,c2X_0,c2Y_0,\dots,c1X_{N-1},c1Y_{N-1},c2X_{N-1},c2Y_{N-1})
\end{equation}

The vote proof uses $(Com, H_{ct}, pk_X, pk_Y)$ as public signals and proves consistency between commitment, candidate index, and encrypted one-hot structure.
...
\begin{itemize}
    \item key ownership consistency: $PK = sk \cdot G$;
...
\begin{equation}
node = keccak256(left \parallel right)
\end{equation}
