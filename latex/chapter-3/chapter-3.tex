...
Adversarial administrator & Refuse tally or submit inconsistent off-chain artifacts & Availability/auditability & On-chain state machine + proof gates & Denial-of-service and off-chain artifact inconsistency not fully eliminated \\
...
\end{equation}
where $r_j$ is sampled uniformly from the BabyJubJub scalar field, $G$ is generator, and $\mathit{PK}$ is administrator public key. The notation is general in $N$, while the current deployed prototype instantiates $N=2$ and enforces this cardinality at contract layer.
