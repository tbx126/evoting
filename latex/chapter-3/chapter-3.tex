...
The model considers malicious voters and passive blockchain observers. For administrator behavior, the prototype assumes an \emph{honest-but-curious} operator for key custody: the administrator is expected to follow the protocol, while single-key control remains an explicit trust assumption rather than a cryptographically enforced guarantee. The following attacks are explicitly targeted:
\begin{itemize}
...
In the current prototype, ZKP3 binds correctness of result \emph{points}. The contract additionally publishes these result points from ZKP public signals so auditors can verify consistency between published integers and point-form tally evidence; however, per-candidate integer-to-point binding is not enforced by contract constraints.
...
After tally finalization, the contract rejects \texttt{merkleRoot} updates, preserving the audit digest as immutable state.
